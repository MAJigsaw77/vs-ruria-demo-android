...
class PlayState extends MusicBeatState
{
	...
	override public function create()
	{
		...
		switch (SONG.song.toLowerCase())
		{
							case 'safe-trip-minus' | 'rough-play-minus':
							{
								curStage='minusRuria';
								defaultCamZoom = .85;
							...
							case 'safe-trip' | 'tutorial':
							{
								...
								var cool = new FlxSprite(-600,-200).loadGraphic(Paths.image("bgfilter"));
								cool.antialiasing=true;
								cool.scrollFactor.set(1,1);
								cool.active=false;
								add(cool);
								...
								snowStorm.frames = Paths.getSparrowAtlas("strongsnowstorm");
								...
								snowStorm.animation.addByPrefix("idle","Snow weak instance",24,true);
								snowStorm.alpha=0;
		...
		if(SONG.player1=='bf-neb')
			gfVersion = 'lizzy';
		if(SONG.player1=='bf' && SONG.player2=='ruria-' || SONG.player1=='bf-')
			gfVersion='gf-';
		...
		boyfriend = new Boyfriend(770, 450, (SONG.player1=='bf' && SONG.player2=='ruria-')?'bf-':SONG.player1);
		...
		switch(SONG.player1){
			...
			case 'ruria' | 'ruria-':
				p1Color = 0xFFFF6036;
			case 'bf' | 'bf-car' | 'bf-' | 'bf-pixel' | 'bf-christmas' | 'bf2':
				p1Color = 0xFF31B0D1;
			default:
				p1Color = 0xFF66FF33;
		}
	...
	private function generateSong(dataPath:String):Void
	{
		...
		stormSound = new FlxSound().loadEmbedded(Paths.sound("snowstorm"));
		stormSound.looped=true;
		vocals.looped=false;

		FlxG.sound.list.add(vocals);
		FlxG.sound.list.add(stormSound);
		...
		for (section in noteData)
		{
			...
			for (songNotes in section.sectionNotes)
			{
				...
				var swagNote:Note = new Note(daStrumTime, daNoteData, oldNote, false, songNotes[3]==null?0:songNotes[3]);
				swagNote.gfSings = gfSings;
				...
				for (susNote in 0...Math.floor(susLength))
				{
					...
					var sustainNote:Note = new Note(daStrumTime + (Conductor.stepCrochet * susNote) + Conductor.stepCrochet, daNoteData, oldNote, true, songNotes[3]==null?0:songNotes[3]);
					sustainNote.scrollFactor.set();
					sustainNote.gfSings = gfSings;
	...
	override public function update(elapsed:Float)
	{
		...
		if (generatedMusic)
		{
			...
			renderedNotes.forEachAlive(function(daNote:Note)
			{
				...
				{
					if((daNote.mustPress || !daNote.mustPress && !currentOptions.middleScroll) && !daNote.gfSings){
						daNote.visible = true;
					}
				...
				if (!daNote.mustPress && daNote.canBeHit && !daNote.wasGoodHit)
				{
					...
					updateHP(-modchart.opponentHPDrain);
						...
						var canHold = daNote.isSustainNote && singer.animation.getByName(anim+"Hold")!=null;
						if(canHold && !singer.animation.curAnim.name.startsWith(anim)){
	...
	function killNoteHit(note:Note):Void
	{
		combo=0;
		totalNotes++;
	...
	var nextStorm = FlxG.random.float(10,15);
	...
	override function beatHit()
	{
		...
		{
			case 'ruriaSnow':
				if(cumpissstormTimer>nextStorm){
					cumpissstormTimer=0;
					nextStorm=FlxG.random.float(10,25);
					stormSound.play(true,0);
					snowStorm.visible=true;
					FlxTween.tween(snowStorm, {alpha: 1}, .5, {
						ease: FlxEase.quadInOut,
						onComplete: function(twn:FlxTween)
						{
							new FlxTimer().start(FlxG.random.float(5, 8), function(tmr:FlxTimer){
								stormSound.fadeOut(.5, 0, function(twn:FlxTween)
								{
									stormSound.pause();
								});
								FlxTween.tween(snowStorm, {alpha: 0}, .5, {
									ease: FlxEase.quadInOut,
									onComplete: function(twn:FlxTween)
									{
										snowStorm.visible=false;
									}
								});
							});
						}
